...
!function() {
    ...
    // let 調色盤（ API: iro.js ）
    let paintPicker = new iro.ColorPicker( '#PaintPicker' , {
        width:     _paintWidth,
    ...
    function paintClick() {
        ...
        let n = _paintArray[ this.getAttribute( 'value' ) ],
            d = n[ 'Id' ];
    ...
    btnSend.onclick = () => {
        ...
        if ( l >= 4 && l <= 20 ) {
            ...
            let p = i => paintPointers[ i ].getAttribute( 'data-color-save' ),
    ...
    paintPointers.forEach( el => {
        el.onclick = () => {
            ...
            let c = el.getAttribute( 'data-color-set' ),
...
!function() {
    ...
    // func 開啟全屏
    function skillOpen() {
        let n = _skillArray[ this.getAttribute( 'value' ) ],
            t = n[ 'Title' ],
            d = n[ 'Desc' ],
            s = n[ 'Score' ],
            c = skillColor( s );
